...
<br>
<p>
    فئة أ: من 90% فأكثر للأقسام العلمية في الثانوية العامة أو ما يعادلها، و80% فأكثر للقسم الأدبي (تنطبق فقط على تخصصات إدارة الأعمال المعلوماتية وفنون التصميم الرقمي)
    <br>
    فئة ب: من 80% إلى أقل من 90% للأقسام العلمية في الثانوية العامة، ومن 70% إلى أقل من 80% للقسم الأدبي (تنطبق فقط على تخصصات إدارة الأعمال المعلوماتية وفنون التصميم الرقمي)
    <br>
    فئة ج: أقل من 80% للأقسام العلمية في الثانوية العامة، وأقل من 70% للقسم الأدبي
      </p>
      <br>
...
export default {
    ...
    methods: {
        async get_data() {
            try {
                const response = await axios.get('https://nuft-website-backend.vercel.app/nationalfaculty/getnational');
                ...
                if (Array.isArray(this.all_data)) {
                    ...
                    this.filteredData = this.all_data.filter(index => index.university === 'EUI' && index.normal_or_Dual === 'normal');
                    this.filteredInternational = this.all_data.filter(index => index.normal_or_Dual === 'dual' && index.university === 'EUI');
        ...
        async getLinks() {
            try {
                const response = await axios.get('https://nuft-website-backend.vercel.app/nationallinks/get_nationallinks');
                this.links = response.data;
                this.filteredLinks = this.links.filter(index => index.university === 'EUI');
        ...
        async getDorms() {
            try {
                const response = await axios.get('https://nuft-website-backend.vercel.app/nationaldorms/getnationaldorms');
        ...
        async gettrans() {
            try {
                const response = await axios.get('https://nuft-website-backend.vercel.app/nationaltrans/getnationaltrans');
