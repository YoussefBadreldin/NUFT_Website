...
<template>
  <div>
    ...
    <section>
      ...
      <br>
      <p>
        الرسوم المذكورة تشمل مصاريف النقل ولا تشمل رسوم التقديم (إن وجدت)، المصاريف الإدارية (إن وجدت)، تأمين المعاملات (إن وجد)، رسوم الخدمات التعليمية (إن وجدت)، مصاريف الإقامة (إن وجدت)
      </p>
    ...
    methods: {
        async get_data() {
            try {
                ...
                if (Array.isArray(this.all_data)) {
                    ...
                    this.filteredData = this.all_data.filter(index => index.university === 'BADYA' && index.normal_or_Dual === 'normal');
                    this.filteredInternational = this.all_data.filter(index => index.normal_or_Dual === 'dual' && index.university === 'BADYA');
        ...
        async getLinks() {
            try {
                ...
                this.filteredLinks = this.links.filter(index => index.university === 'BADYA');
