    ...
    <div>
        ...
        <section>
            <div style="display: flex; flex-direction: column; align-items: center;">
                <img src="/images/Logos/Universites/Private/MTI.png" alt="University Logo" style="width: 100%; max-width: 300px; height: auto;">
                
                <!-- ######## Uni Title ####### -->
                <h2 style="text-align: center; margin-top: 20px;">الجامعة الحديثة للتكنولوجيا والمعلومات<br>(MTI)</h2>
                
                <!-- ######## Uni info ####### -->
                <p style="text-align: center;">
                    جامعة أهلية منبثقة عن جامعة الاسكندرية الحكومية 
                    <br>
                     أنشئت وفقًا للقرار الجمهوري رقم 425 لسنة 2022
                </p>
                
    <!-- ######## Uni Location ####### -->
    <p style="text-align: center;">تقع على مساحة 30 فدانا، بمدينة أبيس (سموحة الجديدة)، محافظة الإسكندرية ويضم مناطق رياضية وترفيهية </p>
    <a href="https://goo.gl/maps/SjP16WQvXSJ2MF476?coh=178573&entry=tt" style="display: block; text-align: center;">
...
export default {
    ...
    methods: {
        async get_data() {
            try {
                const response = await axios.get('https://nuft-website-backend-874bbf91403c.herokuapp.com/Privatefaclity/getPrivate');
                ...
                if (Array.isArray(this.all_data)) {
                    ...
                    this.filteredData = this.all_data.filter(index => index.speciality === 'MTI' && index.facality_or_international === 'facality');
                    this.filteredInternational = this.all_data.filter(index => index.facality_or_international === 'international' && index.speciality === 'MTI');
        ...
        async getLinks() {
            try {
                ...
                this.filteredLinks = this.links.filter(index => index.university_Name === 'MTI');
        ...
        async getDorms() {
            try {
                ...
                this.filteredDorms = this.dorms.filter(dorm => dorm.spec === 'MTI');
        ...
        async gettrans() {
            try {
                ...
                this.filteredTrans = this.trans.filter(index => index.spec === 'MTI');
