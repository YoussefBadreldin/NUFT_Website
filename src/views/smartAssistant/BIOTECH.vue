<template>
  <div>
    <HeaderComponent />
    <section>
      <h1>كلية التكنولوجيا الحيوية</h1>
  ...
  methods: {
    async getData() {
      try {
        ...
        this.filteredData = this.all_data.filter(item => item.faculty_short === 'BIOTECH' && item.normal_or_Dual === 'normal');
        this.filteredInternational = this.all_data.filter(item => item.faculty_short === 'BIOTECH' && item.normal_or_Dual === 'dual');
