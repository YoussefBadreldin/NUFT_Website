  ...
  methods: {
    async getData() {
      try {
        ...
        this.filteredData = this.all_data.filter(item => item.faculty_short === 'ENG' && item.normal_or_Dual === 'normal');
        this.filteredInternational = this.all_data.filter(item => item.faculty_short === 'ENG' && item.normal_or_Dual === 'dual');
      } catch (error) {
